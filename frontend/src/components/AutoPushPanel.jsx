...
import { Clock, Play, XCircle, History, Zap, Timer, CheckCircle, AlertCircle, RefreshCw } from 'lucide-react';

const API_BASE = '/api';

function AutoPushPanel({ workspacePath, repoData, onLog, isActive = false }) {
  ...
  // ✅ Load initial data ONLY when panel becomes active
  useEffect(() => {
    if (!isActive || !workspacePath || !repoData) {
      return;
    }
    ...
    // Start controlled polling only when panel is active AND visible
    loadActiveJobs();
    
    // Set up interval - only while on this tab
    intervalRef.current = setInterval(() => {
      loadActiveJobs();
    }, 30000); // 30 seconds - reasonable polling interval
  ...
  }, [isActive, workspacePath, repoData]);

  const loadActiveJobs = async () => {
    if (isLoadingJobs) return; // Prevent duplicate calls
    
    try {
      setIsLoadingJobs(true);
      // Note: /api/repo/auto-push/active endpoint doesn't exist yet
      // For now, just use the history endpoint to get recent jobs
      const response = await fetch(`${API_BASE}/repo/auto-push/history`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ workspacePath, limit: 5 })
      });

      if (!response.ok) {
        console.error(`Failed to load active jobs: HTTP ${response.status}`);
        return;
      }
  ...
  const loadHistory = async () => {
    try {
      ...
      if (!response.ok) {
        console.error(`Failed to load history: HTTP ${response.status}`);
        return;
      }
  ...
  const loadStats = async () => {
    try {
      ...
      if (!response.ok) {
        console.error(`Failed to load stats: HTTP ${response.status}`);
        return;
      }
      ...
      <div className="card">
        <h2 className="text-lg font-semibold text-white flex items-center justify-between">
          ...
          <button 
            ...
            className="p-1 hover:bg-gray-700 rounded-lg disabled:opacity-50"
            title="Refresh stats"
          >
